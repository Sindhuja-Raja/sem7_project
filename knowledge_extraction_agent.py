...
class PaperKnowledgeExtractor:
    ...
    def extract(self, paper: Paper, paper_index: PaperIndex) -> ExtractionResult:
        ...
        if not outcome.context.strip() or not llm.is_available() or llm.is_daily_quota_exhausted():
            reason = (
                "no LLM configured" if not llm.is_available()
                else "daily Groq quota exhausted" if llm.is_daily_quota_exhausted()
                else "no retrievable context"
            )
            logger.info("Knowledge extraction for %s: %s", paper.title[:60], reason)
            return result
        ...
        try:
            content = llm._call_groq(
                ...
                max_tokens=1800,  # Groq counts this reservation against the account's TPM budget
                                  # regardless of actual usage. Raised from the original 1500 (live
                                  # testing showed gpt-oss-120b spends real completion-token budget
                                  # on a hidden `reasoning` field before writing the 28-field JSON
                                  # answer, and 1500 reliably hit Groq's "max completion tokens
                                  # reached before generating a valid document" error), then
                                  # brought back down from a since-reverted 2200 after THAT caused
                                  # real 413 "Request too large" errors instead - 2200 combined
                                  # with a 12-chunk context left no headroom under this account's
                                  # confirmed 8,000-token-per-request cap. reasoning_effort="low"
                                  # (llm.py default) cuts the reasoning overhead that made 1500
                                  # insufficient in the first place; 1800 + the reduced 9-chunk cap
                                  # (see config.RAG_MAX_CHUNKS_KNOWLEDGE_COMBINED) is the balance
                                  # confirmed to fit with real headroom on a live run.
                temperature=0.1,
                json_mode=True,
                agent_name="KnowledgeExtraction",
                paper_id=paper.title,
                cache_version=config.LLM_CACHE_VERSION,
                rag_chunk_count=len(outcome.retrieved_chunks),
            )
        except llm.GroqDailyQuotaExceeded as exc:
            logger.warning("Knowledge extraction for %s: daily Groq quota exhausted - %s", paper.title[:60], exc)
            return result
