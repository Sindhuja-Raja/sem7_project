...
def get_full_paper_text(paper: Paper) -> Dict[str, str]:
    ...
    def _cached_abstract_fallback() -> Dict[str, str]:
        """Same fallback as branch 2 above, but reached after a PDF fetch
        was actually ATTEMPTED and failed (403/timeout/non-PDF/no usable
        text) - this branch previously returned the fallback WITHOUT ever
        writing it to cache_path, unlike branch 2. CONFIRMED real-world
        impact: a paper whose PDF host blocks automated downloads (e.g. a
        403 from ssrn.com/doi.org) re-attempted the SAME failing download
        on every single subsequent search/rerun instead of reusing the
        abstract it already had, and left get_full_paper_text() unable to
        serve this paper from cache at all - every downstream stage
        (validation, chunking, RAG) redid its own work from scratch too."""
        fallback = _abstract_fallback(paper)
        if fallback:
            try:
                cache_path.write_text(_sections_to_document(fallback), encoding="utf-8")
            except OSError:
                pass
        return fallback
    ...
    try:
        ...
        return _cached_abstract_fallback()
