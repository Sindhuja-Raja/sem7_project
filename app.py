...
def render_problem_solution_analysis(analysis) -> None:
    ...
    if not analysis.problem_solution_table:
        # Show WHY this is empty (quota exhausted / retrieval failure /
        # LLM extraction failure / genuinely insufficient evidence)
        # instead of one generic message for every possible cause -
        # see pipeline_status.py.
        # getattr(..., default) rather than direct attribute access:
        # `analysis` may be a stale ProblemSolutionAnalysis instance
        # already sitting in st.session_state from BEFORE these two
        # fields were added to the dataclass - Streamlit's autoreload
        # re-imports the module but never retrofits already-constructed
        # objects, so a plain .status_reason access can raise
        # AttributeError against old session state until the next fresh
        # search rebuilds it. Falls back to SUCCESS (no special message)
        # rather than crashing.
        status = getattr(analysis, "status", pipeline_status.SUCCESS)
        reason = getattr(analysis, "status_reason", "") or pipeline_status.MESSAGES.get(status, "")
        if status == pipeline_status.DAILY_QUOTA_EXCEEDED:
            st.warning(f"⏳ {reason}")
        elif status == pipeline_status.LLM_UNAVAILABLE:
            st.warning(f"🔑 {reason}")
        elif status in (pipeline_status.RETRIEVAL_FAILURE, pipeline_status.EXTRACTION_FAILURE):
            st.error(f"⚠️ {reason}")
        else:
            st.info(reason or "No data extracted for this section.")
        return
...
def render_contradiction_detection(papers: List[Paper]) -> None:
    ...
    if detect_clicked:
        if len(selected_labels) < 2:
            ...
            with st.status("Running contradiction detection...", expanded=True) as status:
                ...
                claims_by_paper, claims_status, claims_reason = extract_claims(
                    selected_papers, selected_labels, rag_cache,
                )
                ...
                if claims_status != pipeline_status.SUCCESS and total_claims == 0:
                    status.write(f"⚠️ {claims_reason}")

                empty_labels = [selected_labels[i] for i, c in claims_by_paper.items() if not c]
                if empty_labels and claims_status == pipeline_status.SUCCESS:
                    status.write(
            ...
            # Report exactly why there are 0 comparisons when that's the
            # case, instead of only ever showing a generic empty state:
            # claim extraction may have failed/been blocked (see status
            # above), or claims came back fine but genuinely didn't match
            # up (a valid "no comparable topics" result, distinct from
            # "no contradiction was found among comparable claims").
            if comparisons:
                report_status, report_reason = pipeline_status.SUCCESS, ""
            elif claims_status != pipeline_status.SUCCESS:
                report_status, report_reason = claims_status, claims_reason
            elif total_claims > 0 and not matched_pairs:
                report_status, report_reason = pipeline_status.INSUFFICIENT_EVIDENCE, (
                    "Claims were extracted successfully, but none discussed a similar enough topic "
                    "across the selected papers to compare - a valid result, not a failure."
                )
            else:
                report_status, report_reason = pipeline_status.INSUFFICIENT_EVIDENCE, (
                    "No checkable scientific claims could be extracted from the selected papers."
                )
    ...
    if not table:
        # Show WHY there's nothing to compare instead of one generic
        # message for every cause - see pipeline_status.py. getattr with
        # a default guards against a stale ContradictionReport already in
        # st.session_state from before these fields existed (see the
        # matching comment in render_problem_solution_analysis above).
        status = getattr(report, "status", pipeline_status.SUCCESS)
        reason = getattr(report, "status_reason", "") or pipeline_status.MESSAGES.get(status, "")
        if status == pipeline_status.DAILY_QUOTA_EXCEEDED:
            st.warning(f"⏳ {reason}")
        elif status == pipeline_status.LLM_UNAVAILABLE:
            st.warning(f"🔑 {reason}")
            ...
            st.info(reason or "No comparable claim pairs were found across the selected papers.")
    else:
