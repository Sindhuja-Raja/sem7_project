...
def _extract_problems(papers: List[Paper], rag_cache: Dict[str, PaperIndex]) -> List[PaperProblems]:
    """One entry per paper, same order. Falls back to "Not Found in
    Retrieved Context" / empty secondary list if the LLM is unavailable
    or unparsable."""
    empty = [PaperProblems() for _ in papers]
    if not papers or not llm.is_available():
        return empty
...
def _label_problem_clusters(clusters: List[List[Tuple[str, str]]]) -> List[str]:
    ...
    if not clusters or not llm.is_available():
        return fallback
...
def analyze_problems_and_solutions(
    ...
    return ProblemSolutionAnalysis(problem_solution_table=public_table, summary=summary)
