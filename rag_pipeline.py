...
# app.py's paper-collection loop runs build_paper_index_from_sections()
# for up to 8 candidate papers CONCURRENTLY (ThreadPoolExecutor), to
# overlap the truly independent, I/O-bound PDF download/extraction for
# each paper. But chunking (tokenizer.encode, for token-length checks)
# and embedding (model.encode) both call into the SAME cached
# SentenceTransformer instance (get_chunk_embedding_model() below is a
# st.cache_resource singleton, by design - loading a ~100MB+ model once
# per paper would be far worse) - and HuggingFace's fast (Rust-backed)
# tokenizers are NOT safe to call concurrently from multiple threads on
# one instance. CONFIRMED live: this produced real "Already borrowed"
# RuntimeErrors that silently dropped otherwise-valid papers out of the
# RAG pipeline entirely (build_paper_index_from_sections raising ->
# app.py's except-clause marks that paper INVALID). This lock serializes
# only the actual tokenizer/model-touching work (chunk_sections + _embed
# below), not the PDF fetch that dominates each paper's wall-clock time,
# so the ThreadPoolExecutor in app.py still gets real overlap where it
# matters.
...
class PaperIndex:
    ...
    source_label: str = "Not Available"  # e.g. "RAG (14 chunks from Full PDF)",
                                          ...
                                          # "Not Available"
    # RAG RESULT CACHE - keyed by the exact (queries, max_total_chunks)
    # asked of retrieve_context() for THIS paper. This PaperIndex instance
    # lives in st.session_state.rag_index_cache for the life of a search
    # result set, so a Streamlit rerun (or a second agent asking the same
    # question - e.g. Problem-Solution and Root Cause both retrieve
    # "methodology"-flavored context) reuses the already-computed
    # retrieval instead of re-embedding the query and re-searching FAISS.
    # Does not persist across a NEW search (a fresh PaperIndex is built
    # then) - that's correct, since a new search can mean new chunks.
    _retrieval_cache: Dict[tuple, Tuple[str, List["RetrievedChunk"]]] = field(default_factory=dict, repr=False)
...
def _embed(texts: List[str], is_query: bool) -> np.ndarray:
    ...
    similarity for FAISS). Guarded by _tokenizer_lock (see its module-
    level docstring) - this is THE shared choke point every caller
    (paper indexing AND every agent's per-query retrieval) goes through,
    so locking here protects all of them regardless of which higher-level
    code path is calling it or how many ThreadPoolExecutor workers are
    configured now or in the future."""
...
def build_paper_index_from_sections(paper: Paper, sections: Dict[str, str]) -> PaperIndex:
    ...
    # chunk_sections (tokenizer.encode, for token-length checks) touches
    # the same shared, non-thread-safe SentenceTransformer instance as
    # _embed() - and this function is called concurrently across papers
    # by app.py - so it's serialized here too. _embed() below guards
    # itself (see its own docstring/_tokenizer_lock) - NOT re-acquired
    # here as well, since threading.Lock is non-reentrant and doing so
    # would deadlock this thread against its own already-held lock.
    with _tokenizer_lock:
...
def retrieve_context(
    ...
    RAG RESULT CACHE: identical (queries, max_total_chunks) against the
    SAME paper_index (e.g. a Streamlit rerun re-invoking the same agent,
    or two different agents that happen to ask the same retrieval
    question) is served from paper_index._retrieval_cache instead of
    re-embedding the query and re-searching FAISS - the embedding model
    call is the actual cost here, not the (cheap) FAISS search itself.
    """
    cache_key = (tuple(queries), max_total_chunks)
    cached = paper_index._retrieval_cache.get(cache_key)
    ...
    result = (assembled, ranked)
    paper_index._retrieval_cache[cache_key] = result
    return result
