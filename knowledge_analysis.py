...
def _label_clusters(clusters: List[List[Tuple[str, str]]], system_prompt: str, agent_name: str) -> List[dict]:
    if not clusters:
        return []
    if not llm.is_available() or llm.is_daily_quota_exhausted():
    ...
    try:
        ...
        logger.warning("%s: daily Groq quota exhausted - %s", agent_name, exc)
        return [{} for _ in clusters]
...
def _build_research_gap_table(rows: List[dict]) -> List[dict]:
    ...
    labels = _label_clusters(clusters, GAP_LABELING_SYSTEM_PROMPT, agent_name="ResearchGapLabeling")
    table = []
...
def _build_future_work_table(rows: List[dict]) -> List[dict]:
    ...
    labels = _label_clusters(clusters, FUTURE_WORK_LABELING_SYSTEM_PROMPT, agent_name="FutureWorkLabeling")
    table = []
    for cluster, label_entry in zip(clusters, labels):
        papers = ", ".join(sorted({title for title, _ in cluster}))
...
def _generate_insights(
    ...
    if llm.is_daily_quota_exhausted():
        return [{"Insight Category": c, "Insight": "Daily Groq token quota exhausted - insights unavailable."}
                for c in INSIGHT_CATEGORIES]
    ...
    try:
        content = llm._call_groq(
            messages=[
                ...
                # Compact (no indent) - read by the LLM, not a human; the extra
                # whitespace of pretty-printing is pure wasted tokens on this
                # account's tight per-request budget (see llm.py's TPM notes).
                {"role": "user", "content": json.dumps(summary, separators=(",", ":"))},
            ],
            max_tokens=900,
            temperature=0.2,
            json_mode=True,
            agent_name="ResearchInsights",
            paper_id=f"papers[{paper_count}]",
            cache_version=config.LLM_CACHE_VERSION,
        )
    except llm.GroqDailyQuotaExceeded as exc:
        logger.warning("Research insights: daily Groq quota exhausted - %s", exc)
        return [{"Insight Category": c, "Insight": "Daily Groq token quota exhausted - insights unavailable."}
                for c in INSIGHT_CATEGORIES]
    data = llm.parse_json(content) if content else None
