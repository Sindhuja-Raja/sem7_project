...
def extract_claims(
    ...
    """Returns {paper index (0-based) -> claims}. Empty per-paper lists if
    the LLM is unavailable or its output can't be parsed/matched. `rag_cache`
    is the shared {paper title -> PaperIndex} built once in app.py."""
    empty = {i: [] for i in range(len(papers))}
    if not llm.is_available() or not papers:
        return empty
...
def match_claim_pairs(claims_by_paper: Dict[int, List[Claim]]) -> List[Tuple[Claim, Claim, float]]:
    ...
    return pairs[: config.CONTRADICTION_MAX_CLAIM_PAIRS]
...
def classify_pairs(pairs: List[Tuple[Claim, Claim, float]]) -> List[ClaimComparison]:
    ...
    if not llm.is_available():
        return [
            ClaimComparison(
                ...
                reason="GROQ_API_KEY not configured - classification unavailable.",
                supporting_evidence=[],
            )
            for a, b, _ in pairs
        ]
    ...
    content = llm._call_groq(
        ...
        max_tokens=min(4000, 300 * len(pairs)),
        temperature=0.1,
        json_mode=True,
    )
    data = llm.parse_json(content) if content else None
